...
echo "Summary (t2v)"
echo "=============================================="
printf "%-45s  %-6s  %-6s  %-6s  %-6s  %-6s  %-6s  %-5s  %-6s\n" "checkpoint" "R@1" "R@5" "R@10" "R@20" "R@30" "R@50" "MdR" "mAP"
printf "%-45s  %-6s  %-6s  %-6s  %-6s  %-6s  %-6s  %-5s  %-6s\n" "---------------------------------------------" "------" "------" "------" "------" "------" "------" "-----" "------"

# Include baseline if present
baseline="$OUT_DIR/eval_baseline.json"
if [[ -f "$baseline" ]]; then
  row=$(jq -r '.text_to_video | "\(."R@1")\t\(."R@5")\t\(."R@10")\t\(."R@20")\t\(."R@30")\t\(."R@50")\t\(.MdR)\t\(.mAP)"' "$baseline")
  IFS=$'\t' read -r r1 r5 r10 r20 r30 r50 mdr map <<< "$row"
  printf "%-45s  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-5.1f  %-6.4f\n" "baseline (zero-shot)" "$r1" "$r5" "$r10" "$r20" "$r30" "$r50" "$mdr" "$map"
fi
...
for ck in "${CHECKPOINTS[@]}"; do
  ...
  out_json="$OUT_DIR/eval_${parent}_${tag}.json"
  [[ -f "$out_json" ]] || continue
  row=$(jq -r '.text_to_video | "\(."R@1")\t\(."R@5")\t\(."R@10")\t\(."R@20")\t\(."R@30")\t\(."R@50")\t\(.MdR)\t\(.mAP)"' "$out_json")
  IFS=$'\t' read -r r1 r5 r10 r20 r30 r50 mdr map <<< "$row"
  printf "%-45s  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-5.1f  %-6.4f\n" "$parent/$tag" "$r1" "$r5" "$r10" "$r20" "$r30" "$r50" "$mdr" "$map"
done
...
if [[ -f "$baseline" ]]; then
  row=$(jq -r '.video_to_text | "\(."R@1")\t\(."R@5")\t\(."R@10")\t\(."R@20")\t\(."R@30")\t\(."R@50")\t\(.MdR)\t\(.mAP)"' "$baseline")
  IFS=$'\t' read -r r1 r5 r10 r20 r30 r50 mdr map <<< "$row"
  printf "%-45s  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-5.1f  %-6.4f\n" "baseline (zero-shot)" "$r1" "$r5" "$r10" "$r20" "$r30" "$r50" "$mdr" "$map"
fi
...
for ck in "${CHECKPOINTS[@]}"; do
  ...
  out_json="$OUT_DIR/eval_${parent}_${tag}.json"
  [[ -f "$out_json" ]] || continue
  row=$(jq -r '.video_to_text | "\(."R@1")\t\(."R@5")\t\(."R@10")\t\(."R@20")\t\(."R@30")\t\(."R@50")\t\(.MdR)\t\(.mAP)"' "$out_json")
  IFS=$'\t' read -r r1 r5 r10 r20 r30 r50 mdr map <<< "$row"
  printf "%-45s  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-6.4f  %-5.1f  %-6.4f\n" "$parent/$tag" "$r1" "$r5" "$r10" "$r20" "$r30" "$r50" "$mdr" "$map"
done
