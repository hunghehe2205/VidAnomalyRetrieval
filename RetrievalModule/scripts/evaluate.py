...
from RetrievalModule.src.var.data import QueryVideoDataset, build_positive_groups
from RetrievalModule.src.var.iolog import log, new_log_filename, tee_to_file
from RetrievalModule.src.var.metrics import rank_positions, summarize
from RetrievalModule.src.var.model import QwenEmbeddingEngine, load_adapter
...
def _run(args: argparse.Namespace) -> Path:
    ...
    if args.adapter is not None:
        ...
        mode = "zero-shot"
    log("eval", f"mode: {mode}")
    log("eval", f"device: {engine.device}")
...
def main() -> None:
    ...
    stem = "eval_baseline" if args.zero_shot else "eval"
    log_path = log_dir / new_log_filename(stem)
    with tee_to_file(log_path):
        log("eval", f"log file: {log_path}")
        _run(args)
        log("eval", "done.")
